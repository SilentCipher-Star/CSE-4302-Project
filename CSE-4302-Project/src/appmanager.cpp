...
void AcadenceManager::reindexSearch()
{

    QString studentPath = "C:/Users/HP/Documents/OOP_PROJECT/CSE-4302-Project/data/students.csv";
    QString teacherPath = "C:/Users/HP/Documents/OOP_PROJECT/CSE-4302-Project/data/teachers.csv";

    QVector<QStringList> studentData = CsvHandler::readCsv(studentPath);
    
    std::cout << "\n--- SEARCH INDEXING START ---" << std::endl;
    std::cout << "Attempting to load: " << studentPath.toStdString() << std::endl;
    std::cout << "Students loaded: " << studentData.size() << std::endl;

    for (const auto &row : studentData) {
        if (row.size() > 1) {
            std::string name = row[1].trimmed().toLower().toStdString();
            globalSearchTrie.insert(name, "Student");
        }
    }

    QVector<QStringList> teacherData = CsvHandler::readCsv(teacherPath);
    std::cout << "Teachers loaded: " << teacherData.size() << std::endl;


    for (const auto &row : teacherData) {
        if (row.size() > 1) {
            std::string name = row[1].trimmed().toLower().toStdString();
            globalSearchTrie.insert(name, "Teacher");
        }
    }

    std::cout << "--- INDEXING FINISHED ---\n" << std::endl;

    // 2. Index Courses from CSV
    QVector<QStringList> courseData = CsvHandler::readCsv("data/courses.csv");
    for (const auto &row : courseData)
    {
        if (row.size() > 1)
        {
            std::string courseTitle = row[1].trimmed().toLower().toStdString();
            globalSearchTrie.insert(courseTitle, "Course");
        }
    }

    // 3. Index existing Notices
    for (const auto &notice : getNotices())
    {
        std::string preview = notice.getContent().left(30).toLower().toStdString();
        globalSearchTrie.insert(preview, "Notice");
    }
}

QVector<QPair<QString, QString>> AcadenceManager::getGlobalSearchSuggestions(const QString &text) {
    QVector<QPair<QString, QString>> qtResults;
    auto results = globalSearchTrie.search(text.toLower().toStdString());
    for (const auto& res : results) {
        qtResults.append({QString::fromStdString(res.first), QString::fromStdString(res.second)});
    }
    return qtResults;
}