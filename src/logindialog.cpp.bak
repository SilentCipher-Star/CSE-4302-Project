    ...
    : QDialog(parent), m_app(app), userId(-1), currentThemeIdx(0)
{
    ...
    welcomeLabel->setStyleSheet(QString("font-size: %1; font-weight: bold; margin-bottom: 8px; letter-spacing: 1px;").arg(AppFonts::Title));
    mainLayout->addWidget(welcomeLabel, 0, Qt::AlignCenter);

    QLabel *taglineLabel = new QLabel("Your campus. Your schedule. Your success.", this);
    taglineLabel->setAlignment(Qt::AlignCenter);
    taglineLabel->setStyleSheet("font-size: 18px; font-weight: 400; opacity: 0.75; margin-bottom: 28px; letter-spacing: 0.5px;");
    mainLayout->addWidget(taglineLabel, 0, Qt::AlignCenter);

    QFrame *centerFrame = new QFrame(this);
    centerFrame->setFixedWidth(420);
    centerFrame->setObjectName("loginFrame");

    QVBoxLayout *frameLayout = new QVBoxLayout(centerFrame);
    frameLayout->setContentsMargins(48, 48, 48, 48);
    ...
    accentBar->setStyleSheet("background-color: palette(highlight); border-radius: 2px;");
    frameLayout->addWidget(accentBar);

    QLabel *cardTitle = new QLabel("Sign In", centerFrame);
    cardTitle->setAlignment(Qt::AlignCenter);
    cardTitle->setStyleSheet("font-size: 26px; font-weight: 700; margin-top: 8px; margin-bottom: 4px; letter-spacing: 0.5px;");
...
void LoginDialog::updateThemeButton(const AppTheme &t)
{
    ...
    if (bar)
    {
        bar->setStyleSheet(QString("background-color: %1; border-radius: 2px;").arg(t.accent));
    }
}
